...
public class AsyncEnumerableExtensionsTests
{
    ...
    public async Task ToImmutableDictionary_Source_KeyValue_WithComparer()
    {
        var result = await GetStrings()
            .ToImmutableDictionary(x => x.Length, x => x, EqualityComparer<int>.Default, CancellationToken.None);
    ...
    {
        var result = await GetStrings()
            .ToImmutableDictionary(x => x.Length, x => x, CancellationToken.None);
    ...
    {
        var result = await GetStrings()
            .ToImmutableDictionary(x => x.Length, CancellationToken.None);
    ...
    {
        var result = await GetStrings()
            .ToImmutableDictionary(x => x.Length, EqualityComparer<int>.Default, CancellationToken.None);
    ...
    {
        var result = await GetPairs()
            .ToImmutableDictionary(EqualityComparer<int>.Default, CancellationToken.None);
    ...
    //{
    //    Func<Task> act = async () =>
    //        await GetNumbers().ToImmutableDictionary(x => x, x => x, CancellationToken.None);
    ...
    //    Func<Task> act = async () =>
    //        await GetStrings().ToImmutableDictionary(x => x.Length, x => x, cts.Token);
