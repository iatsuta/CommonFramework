...
public class ImmutableExtensionsTests
{
    ...
    public async Task ToImmutableArrayAsync_NullSource_ShouldThrow()
    {
        ...
        await Assert.ThrowsAsync<ArgumentNullException>(() => source.ToImmutableArrayAsync(cancellationToken));
    }
    ...
    public async Task ToImmutableListAsync_NullSource_ShouldThrow()
    {
        ...
        IAsyncEnumerable<int> source = null!;
        await Assert.ThrowsAsync<ArgumentNullException>(() => source.ToImmutableListAsync(cancellationToken));
    }
    ...
    public async Task ToImmutableHashSetAsync_NullSource_ShouldThrow()
    {
        ...
        IAsyncEnumerable<int> source = null!;
        await Assert.ThrowsAsync<ArgumentNullException>(() => source.ToImmutableHashSetAsync(cancellationToken));
    }
}